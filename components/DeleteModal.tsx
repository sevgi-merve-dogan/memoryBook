...
export default function DeleteModal() {
  const { showDeleteModal, setShowDeleteModal, walletAddress, setWalletAddress, confirmPaidDelete } = useStore();
  ...
  // Load balance whenever wallet address or modal visibility changes
  useEffect(() => {
    if (walletAddress && showDeleteModal) {
      getBalance(walletAddress).then(setBalance);
    } else {
      setBalance(null);
    }
  }, [walletAddress, showDeleteModal]);
  ...
  async function handleConnect() {
    ...
    try {
      ...
      console.error('[DeleteModal] connect error:', err);
    } finally {
      setConnecting(false);
    }
  }
  ...
  async function handleFund() {
    ...
    try {
      ...
      console.error('[DeleteModal] fund error:', err);
    } finally {
      setFunding(false);
    }
  }
    ...
    try {
      await signAndSubmit(walletAddress, account =>
        new StellarSdk.TransactionBuilder(account, {
          ...
          .addMemo(StellarSdk.Memo.text('ani-defteri-delete'))
          .setTimeout(30)
          .build() as StellarSdk.Transaction
      );
      console.info('[DeleteModal] Payment successful');
      confirmPaidDelete();
    } catch (e: any) {
      console.error('[DeleteModal] Payment error:', e);
        ...
        <>
          ...
          >
            ...
            <h2 style={{ textAlign: 'center', fontSize: 17, fontWeight: 'bold', color: '#2c2a27', marginBottom: 6, fontFamily: 'Georgia' }}>
              Günlük Silme Kotası Doldu
            </h2>
            <p style={{ textAlign: 'center', fontSize: 12, color: '#6a6460', marginBottom: 18, lineHeight: 1.6 }}>
              Bugün 3 ücretsiz silme hakkını kullandın.<br />
              1 silme işlemi için <strong style={{ color: '#2c2a27' }}>1 XLM</strong> gerekiyor.
            </p>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  {balance === null ? (
                    ...
                    <span style={{ fontWeight: 'bold', color: hasEnough ? '#3a8060' : '#dc2626', fontSize: 14 }}>
                      {balance} XLM
                    </span>
                  )}
                </div>
            ...
            {/* Pay button — only when wallet connected and has enough XLM */}
            {walletAddress && (
              <motion.button
              ...
              >
                {paying ? '⏳ İşlem Yapılıyor...' : hasEnough ? '⭐ 1 XLM Öde ve Sil' : '⭐ 1 XLM Öde ve Sil'}
              </motion.button>
            )}
